# Discord Bot for わなみさん v16.1.0

VTuber育成スクール用Discord自動応答チャットボット - Q&A自動生成・週次配信・隠しキーワード機能付き完全版

## 🆕 Version 16.1.0 新機能

### 🎁 隠しキーワード機能
- **特定のキーワード検出**: ユーザーが特定のキーワードを入力すると特別な応答を返す
- **コンテキスト依存**: ボタンを押した後のみ有効なキーワードを設定可能
- **簡単な追加**: `config/hidden-keywords.js` で新しいキーワードを追加可能
- **例**: レッスン質問ボタン後に「WannaV最高」と入力すると特別なURL表示

### ✨ 画像分析機能強化
- **通常質問でも画像分析対応**: ミッション以外の質問でも画像を添付すると分析
- **サムネイル添削**: 画像を添付して「サムネイルの添削をして」と質問可能
- **デザインフィードバック**: 配信画面、ロゴなどのデザイン評価が可能
...
- **手動補充機能**: APIエンドポイントまたはnpm scriptで補充可能
...
### 定期実行スケジューラー
- **週次タスク**: 毎週火曜日 18:00 (JST) - Discord Webhook送信
...
管理画面では、ソースの登録・編集・無効化・削除、取得状態の確認、AI回答への再反映、既存シートからの一括移行ができます。「回答テスト」では、Discordの通常質問と同じ処理で生成した実際の回答と、その回答に使用した全文意味検索ソースを同時に確認できます。
...
### 概要
特定のキーワードをユーザーが入力すると、特別な応答やURLを返す機能です。
ボタンを押した後のコンテキストに応じて、異なるキーワードを設定できます。

### 使用例
1. **レッスン質問ボタンをクリック**
2. わなみさんにメンションして「**WannaV最高**」と入力
3. 特別なレッスン資料のURLが表示される

### 新しい隠しキーワードの追加方法

#### 1. 設定ファイルを編集
`config/hidden-keywords.js` を開く
...
const HIDDEN_KEYWORDS = [
  {
    keyword: 'WannaV最高',
    requiredContext: 'lesson_question',  // レッスン質問ボタン後のみ有効
    response: '🎉 特別な資料はこちら！\nhttps://...'
  },
  ...
  {
    keyword: 'ひみつのあいことば',
    requiredContext: 'sns_consultation',  // SNS運用相談ボタン後のみ有効
    response: '🎊 SNS運用の秘密資料！\nhttps://...'
  }
];
```
...
- `null`: どのコンテキストでも有効（ボタンを押さなくても動作）
