// services/rag-system.js - RAG(Retrieval-Augmented Generation)システム v2.12.0 (クエリ最適化改善版)
...
- 添付画像がある場合は、画像の内容も評価に含める`;
  ...
  // ✅ v2.12.0: クエリ最適化改善版 + Phase 10のプロンプト強化を継承
  async generateKnowledgeOnlyResponse(userQuery, context = {}) {
    try {
      logger.ai('🔒 知識ベース強制限定モード: 応答生成開始（v2.12.0 クエリ最適化改善版）');
...
もう一度、違う言葉で質問してみてくださいね！✨`;
      ...
      // Phase 10のプロンプト強化を継承: 知識ベースの内容を構築（より詳細に）
      let knowledgeContext = '';
      knowledgeResults.forEach((result, index) => {
        knowledgeContext += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
...
必ず「📚 出典: [資料名]」を最後に書いてください。`;
      ...
      logger.info(`🖼️ OpenAI APIに渡す画像メッセージ: ${imageMessages.length}件`);
      logger.info('🤖 AI応答生成中（v2.12.0 クエリ最適化改善版）...');
      ...
      logger.info('✅ 知識ベース強制限定モード: 応答生成完了（v2.12.0）');
