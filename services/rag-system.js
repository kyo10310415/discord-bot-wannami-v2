// services/rag-system.js - RAG(Retrieval-Augmented Generation)システム v2.13.0 (コンテキスト優先度強化版)
...
- 添付画像がある場合は、画像の内容も評価に含める`;
  ...
  // ✅ v2.13.0: Phase 13の改善を適用（検索1位を3回繰り返す + システムプロンプト強化）
  async generateKnowledgeOnlyResponse(userQuery, context = {}) {
    try {
      logger.ai('🔒 知識ベース強制限定モード: 応答生成開始（v2.13.0 コンテキスト優先度強化版）');
...
もう一度、違う言葉で質問してみてくださいね！✨`;
      ...
      // ✨ Phase 13 (オプションC): システムプロンプトをさらに強化して1位を明示
...
1. **🥇 最重要資料「${topResult.source}」を最優先に使用すること**
2. **検索スコア第1位の資料が最も関連性が高い**
3. **他の資料は補足情報としてのみ使用**
4. **資料にない情報は絶対に使わないこと**
5. **一般知識や常識を使わないこと**
...
📌 資料名: ${topResult.source}
🎯 検索スコア: ${(topResult.score * 100).toFixed(0)}% （全${knowledgeResults.length}件中の第1位）
...
ステップ1: 🥇 **最重要資料「${topResult.source}」を最初に読む**
ステップ2: 質問に関連する部分を**最重要資料から抜き出す**
ステップ3: 必要に応じて他の資料も参照する
ステップ4: 抜き出した内容を**自分の言葉でわかりやすく要約する**
ステップ5: 絵文字を使って**親しみやすく説明する**
ステップ6: 最後に必ず「📚 出典: [${topResult.source}]」を書く

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ 【良い回答の例】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━

質問: 「誰に向けての配信か」を考える上で大切なポイントを教えてください。

良い回答:
「誰に向けての配信か」を考える上で大切なポイントは以下の通りです✨

1. **ターゲットの明確化** 🎯
   配信の対象となる視聴者層（年齢、性別、興味関心）を具体的に設定することが重要です。
...
（理由: 検索1位の資料を無視して、他の資料や一般知識を使っている）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 【今すぐ回答してください】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🥇 **最重要資料「${topResult.source}」の内容を最優先に使って**、
ユーザーの質問に回答してください。
...
必ず「📚 出典: [${topResult.source}]」を最後に書いてください。`;
      ...
      logger.info(`🖼️ OpenAI APIに渡す画像メッセージ: ${imageMessages.length}件`);
      logger.info('🤖 AI応答生成中（v2.13.0 コンテキスト優先度強化版）...');
      ...
      logger.info('✅ 知識ベース強制限定モード: 応答生成完了（v2.13.0）');
